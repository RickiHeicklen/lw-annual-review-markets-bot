...
import { isPing, isPong, ping } from './ping.js';
...
export class SockDaemonClient {
     ...
     */
    async kill() {
        const ps = await readFile(this.#pidFile, 'utf8').catch(() => undefined);
        this.disconnect();
        if (!ps)
            return;
    ...
    async #connect() {
        ...
        connection.on('error', (er) => {
            ...
            if (er.code === 'ENOENT') {
                ...
                const d = spawn(process.execPath, [...ea, this.#daemonScript], {
                    env: {
                        ...process.env,
                        /* c8 ignore start */
                        ...(this.#debug && {
                    ...
                    stdio: [
                        ...
                        //'inherit',
                        //'pipe',
                        openSync(this.#logFile, constants.O_APPEND |
                            constants.O_CREAT |
                            constants.O_WRONLY),
                    ],
                    detached: true,
